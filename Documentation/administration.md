...
In the console it is under **Users → Edit → Set a temporary password…**, behind a confirmation
that names the account and says what it ends. The password then appears in that row, large and
selectable, with a notice saying the console cannot show it again — and the list is deliberately
**not** reloaded, because rebuilding the table would destroy the only copy of it that exists
(D65). The row's status changes in place instead, and the **Status** column carries
`passwordChangeRequired` beside `active`, so an operator can see who is still holding one.
...
- **No server settings screen.** `admin.settings.manage` currently guards reading rather than
  ...
  would need a story about what happens on restart. `MAIL_TRANSPORT` is one of those: switching
  a deployment between sending mail and not is `.env` plus a restart of the API, never a toggle
  here — see [authentication.md](authentication.md).
