...
| **`docker compose exec` is bitten by the same path rewriting, and `--grant-role` is where it shows** | `docker compose exec api /app/launcher-api --grant-role you@example.com admin` from Git Bash fails with `stat C:/Program Files/Git/app/launcher-api: no such file or directory` — which reads as a missing binary in the image rather than as a rewritten argument. Same fix as the row below, `MSYS_NO_PATHCONV=1`, or run it from PowerShell. On the deployment itself, being Linux, it never happens |
| **Files copied in with `docker compose cp` cannot be deleted from inside the container** | `cp` writes them as root and the api container runs as an unprivileged user, so tidying up the three files a `--publish-release` needed answers `rm: cannot remove '/tmp/r.json': Operation not permitted`. `docker compose exec -u root api rm -f …`. Worth remembering because a launcher release is a couple of hundred megabytes sitting in `/tmp` of a container on a small disk |
